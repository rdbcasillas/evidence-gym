...
const props = defineProps<{
  ...
  userClassification: BiasClassification
  ...
  <div class="w-full max-w-lg mx-auto px-4">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            ...
            <template v-else>
              ...
              <strong>{{ props.card.correctClassification === 'biased' ? 'biased' : 'legitimate' }}</strong>
            </template>
          </span>
        </div>
      </div>
      ...
      <div class="p-6 space-y-5">
        ...
        <!-- What's missing (the key insight) -->
        <div
          class="rounded-xl p-4"
          :class="{
            'bg-amber-50 border border-amber-100': props.card.correctClassification === 'biased',
            'bg-green-50 border border-green-100': props.card.correctClassification === 'legitimate',
          }"
        >
          <h3
            class="text-xs font-medium uppercase tracking-wide mb-2"
            :class="{
              'text-amber-700': props.card.correctClassification === 'biased',
              'text-green-700': props.card.correctClassification === 'legitimate',
            }"
          >
            {{ props.card.correctClassification === 'biased' ? 'What You\'re Not Seeing' : 'Why This Is Reliable' }}
          </h3>
          <p
            class="leading-relaxed"
            :class="{
              'text-amber-900': props.card.correctClassification === 'biased',
              'text-green-900': props.card.correctClassification === 'legitimate',
            }"
          >
            {{ props.card.whatsMissing }}
          </p>
        </div>
        ...
        <div class="text-center">
          <span
            ...
            :class="{
              'bg-amber-100 text-amber-700': props.card.correctClassification === 'biased',
              'bg-green-100 text-green-700': props.card.correctClassification === 'legitimate',
            }"
          >
            {{ props.card.tag }}
          </span>
